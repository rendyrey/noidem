					...
					<div class="step-pane active" data-step="1" id="edu_background">
							...
							<div class="col-xs-12 col-sm-5">
								<div class="btn-group" data-toggle="buttons">
									@foreach($tingkat_pendidikan as $value)
										<label class="btn btn-white {{old('id_tingkat_pendidikan')==$value->id ? 'active':''}}">
											<input type="radio" name="id_tingkat_pendidikan" id="{{$value->tingkat}}" autocomplete="off" value="{{$value->id}}" {{old('id_tingkat_pendidikan')==$value->id ? 'checked':''}}> {{$value->tingkat}}
										</label>
									@endforeach
							...
							<div class="col-xs-12 col-sm-5">
								<select name="id_institusi" class="chosen-select form-control" id="id_institusi" data-placeholder="Choose ...">
									...
									@foreach($institusi as $value)
										<option value="{{$value->id}}" {{old('id_institusi')==$value->id ? 'selected':''}}>{{$value->nama_institusi}}</option>
									@endforeach
								</select>
								<br>
								<font color="red">{{$errors->first('id_institusi')}}</font>
							</div>
						</div>
						...
						<div class="form-group">
										...
										@if(is_numeric($key))
											...
											@else
												...
												@foreach ( $attr as $keys => $value )
													<option value="{{$keys}}" {{$keys==old('id_major') ? 'selected':''}}>{{$value}}</option>
												@endforeach
											</optgroup>
										@endforeach
									</select>
									<br>
									<font color="red">{{$errors->first('id_major')}}</font>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-xs-12 col-sm-2">
									<select name="start_year_education" class="form-control">
										<option value="">Select Year</option>
										@foreach ($yearArray as $year)
											<option value="{{$year}}" {{old('start_year_education')==$year ? 'selected':''}}>{{$year}}</option>
										@endforeach
									</select>
									<font color="red">{{$errors->first('start_year_education')}}</font>
								</div>
							</div>
							<div class="form-group">
