...
class JobFairController extends Controller
{
	public function index(){
		...
		$tingkat_pendidikan = DB::table('tingkat_pendidikan')->orderBy('no_urut','ASC')->limit(5)->get();
		$tingkat_pendidikan_more = DB::table('tingkat_pendidikan')->orderBy('no_urut','ASC')->limit(3)->get();
		$dateArray = range(1, 31);
		$months = array(1=>'January','February','March','April','May','June','July ','August','September','October','November','December');
		$cur_year = date("Y");
		$yearArray = range(1985, $cur_year);
		$attributes = array();
		$tanggal_psychotest = TanggalPsychotest::where('tanggal','>=', Carbon::today())->get();
		foreach ( $major as $v ) {
		...
		return view('jobfair.index', compact('bidang_usaha','institusi','kota','major','major_grup','provinsi','dateArray','months','yearArray','cur_year','attributes','tingkat_pendidikan','tingkat_pendidikan_more','tanggal_psychotest'));
	}

	public function index_online(){

		//mengambil loker dengan event code inputan
		$data['bidang_usaha'] = Bidang_usaha::all();
		$data['institusi'] = Institusi::orderBy('nama_institusi','asc')->get();
		$data['kota'] = Kota::all();
		$data['major'] = Major::all();
		$data['major_grup'] = MajorGrup::all();
		$data['provinsi'] = Provinsi::all();
		$data['tingkat_pendidikan'] = DB::table('tingkat_pendidikan')->orderBy('no_urut','ASC')->limit(5)->get();
		$data['tingkat_pendidikan_more'] = DB::table('tingkat_pendidikan')->orderBy('no_urut','ASC')->limit(3)->get();
		$data['dateArray'] = range(1, 31);
		$data['months'] = array(1=>'January','February','March','April','May','June','July ','August','September','October','November','December');
		$data['cur_year'] = date("Y");
		$data['yearArray'] = range(1985, $data['cur_year']);
		$data['attributes'] = array();
		$data['tanggal_psychotest'] = TanggalPsychotest::where('tanggal','>=', Carbon::today())->get();


		foreach ( $data['major'] as $v ) {
			if ( !isset($data['attributes'][$v->major]) ) {
				$data['attributes'][$v->major] = array();
			}
			$data['attributes'][$v->major_grup->nama_grup][$v->id] = $v->major;
		}

		$iklan = Iklan::where('domain', 'jobfair.medion.co.id')->where('actual_end_date', '>=', Carbon::today())->get();
		$data['arr_iklan'] = array();
		foreach ($iklan as $value) {
				if(!isset($data['arr_iklan'][$value->advertising_media->media])) {

						$data['arr_iklan'][$value->id] = array();
				}
				$data['arr_iklan'][$value->advertising_category->kategori][$value->id] = $value->advertising_media->media;
		}
		$data['job_interest'] = Iklan::job_interest_jobfair();
		$data['count_loker'] = count($data['job_interest']);
		if($data['count_loker'] >= 4){
				$data['count_loker'] = 4;
		}else{
				$data['count_loker'] = $data['count_loker'];
		}
		return view('jobfair.form_offline', $data);
	}
	...
	public function tambah(Request $request)
	{
		...
		if($data_exist) {
			...
			return redirect('/jobfair')->with('message','Your application can not be processed, because you have applied');
	...
	public function open_form(Request $request){
		$this->validate($request,[
			'event_code'=>'required|digits:3'
		]);
		$event_code = $request->event_code;
		if(!Iklan::where('event_code',$event_code)->first()){
			return redirect('jobfair')->with('message','Sorry, Event Code is not Available');
		}
		//mengambil loker dengan event code inputan
		$data['loker'] = Iklan::where('event_code',$event_code)->first()->loker()->get();
		$data['id_iklan'] = Iklan::where('event_code',$event_code)->value('id');

		if(count($data['loker'])<4){
			$data['jml_loker'] = count($data['loker']);
		}else{
			$data['jml_loker'] = 4;
		}
