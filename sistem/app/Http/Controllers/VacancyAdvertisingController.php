...
class VacancyAdvertisingController extends Controller
{
  ...
  {
    $iklan_inprocess = Iklan::where('actual_end_date','>=',Carbon::today())->orWhere('actual_end_date','0000-00-00')->get();
    ...
    $iklan_done = Iklan::where('actual_end_date','<',Carbon::today())->where('actual_end_date','<>','0000-00-00')->get();
  ...
  public function tambah_loker($id, Request $request)
  {
    $this->validate($request,[
      ...
      "target_fresh_$id"=>'required|integer',
      "target_exp_$id"=>'required|integer',
      "target_pg_fresh_$id" =>'required|integer',
      "target_pg_exp_$id"=>'required|integer',
      "gender_$id"=>'required',
      ...
      "target_fresh_$id.integer" => "Kolom Target Fresh harus diisi bilangan bulat!",
      "target_exp_$id.required" => "Kolom Target Exp harus diisi!",
      "target_exp_$id.integer" => "Kolom Target Exp harus diisi bilangan bulat!",
      "target_pg_fresh_$id.required"=> "Kolom Target Pg Fresh harus diisi!",
      "target_pg_fresh_$id.integer" => "Kolom Target PG Fresh harus diisi bilangan bulat!",
      "target_pg_exp_$id.required" => "Kolom Target Pg Exp harus diisi!",
      "target_pg_exp_$id.integer" => "Kolom Target PG Exp harus diisi bilangan bulat!",
      "gender_$id.required"=>"Kolom Gender harus diisi!",
  ...
  $loker = new Loker;
  $loker->id_iklan = $id;
  $cost = str_replace('.','',$request->input('cost_'.$id));
  $budget = str_replace('.','',$request->input('budget_'.$id));
  $loker->budget = $budget;
  $loker->cost = $cost;
  $loker->id_tingkat_pendidikan = $request->input('id_tingkat_pendidikan_'.$id);
  $id_major = $request->input('id_major_'.$id,0);
  $loker->id_major_grup = Major::where('id',$id_major)->value('id_grup');
  ...
  return redirect('vacancy_advertising/'.$id.'/detail_iklan')->with('message', 'Data loker berhasil disimpan!')->with('panel','success');
...
{
  $iklan = new Iklan;
  $plan = date('Y-m-d',strtotime('-1 day',strtotime($request->plan_start_date)));
  $this->validate($request,[
    'event_code'=>'required|unique:iklan,event_code',
    'plan_start_date'=>'required|date|after:yesterday',
    // 'id_kategori'=>'required',
    'id_media'=>'required',
    'plan_end_date'=>'required|date|after:'.$plan
  ],[
    'event_code.required'=>'Event code harus diisi!',
    'event_code.unique'=>'Event code sudah dipakai!',
    ...
    'plan_end_date.after'=>'Plan End Date harus lebih dari atau sama dengan Plan Start Date!'
  ]);

  $id_kategori = AdvertisingMedia::where('id',$request->id_media)->value('id_kategori');
  $iklan->event_code = $request->event_code;
  $iklan->id_kategori = $id_kategori;
  ...
  $domain = AdvertisingMedia::where('id',$request->id_media)->value('domain');
  $iklan->domain = $domain;
  $iklan->save();
...
public function update_iklan($id, Request $request)
{
  ...
  $this->validate($request,[
    'event_code'=>'required|exists:iklan,event_code',
    // 'id_kategori'=>'required',
    'id_media'=>'required',
    'plan_start_date'=>'required|date',
    'plan_end_date'=>'required|date|after:plan_start_date',
    'actual_start_date'=>'required|date|in:'.$today,
    'actual_end_date'=>'date|in:'.$today
  ],[
    'event_code.required'=>'Event code harus diisi!',
    // 'id_kategori.required'=>'Adv Category harus diisi!',
    'id_media.required'=>'Adv Media harus diisi!',
    ...
    'actual_start_date.in' => "Kolo Actual Start Date harus diisi sesuai tanggal hari ini!",
    ...
    'actual_end_date.in' => "Kolom Actual End Date harus diisi sesuai tanggal hari ini!",
  ]);
...
public function edit_loker($id)
{
  ...
  $major_grup = MajorGrup::all();
  $major = Major::all();
  $posisi = Posisi::orderBy('posisi','asc')->lists('posisi','id');
  $posisi_publish = Posisi::orderBy('posisi_publish','asc')->groupBy('posisi_publish')->lists('posisi_publish','id');
  $advertising_category = AdvertisingCategory::all()->lists('kategori', 'id');
  $advertising_media = AdvertisingMedia::all();

  return view('admin.vacancy_advertising.edit_loker', compact('loker','tingkat_pendidikan','major_grup','major','advertising_category','advertising_media','posisi','posisi_publish'));
}
...
public function update_loker($id, Request $request)
{
  ...
  $this->validate($request,[
    ...
    "target_exp"=>'required',
    "target_pg_fresh" =>'required',
    "target_pg_exp"=>'required',
    "id_major"=>'required',
    "target_applicant" => "required"
  ],[
    "budget.required" => "Kolom Budget harus diisi!",
    "cost.required" => "Kolom Cost harus diisi!",
    "posisi.required" => "Kolom Posisi harus diisi!",
    "posisi_publish.required" => "Kolom Posisi Publish harus diisi!",
    "target_fresh.required" => "Kolom Target Fresh harus diisi!",
    "target_exp.required" => "Kolom Target Exp harus diisi!",
    "target_pg_fresh.required"=>"Kolom Target Pg Fresh harus diisi!",
    "target_pg_exp.required"=>"Kolom Target Pg Exp harus diisi!",
    "id_major.required"=>"Kolom Major harus diisi!",
    "target_applicant.required" => "Target applicant harus diisi!"
  ]
);
  ...
  $find_grup_id = $request->input('id_major');
  $id_major_grup = Major::where('id',$find_grup_id)->value('id_grup');
  ...
  $target_applicant = $request->input('target_applicant');
  ...
  DB::table('loker')->where('id', $id)->update(['budget' => $budget, 'cost' => $cost,
  'id_tingkat_pendidikan' => $id_tingkat_pendidikan, 'id_major_grup' => $id_major_grup, 'id_major' => $id_major,
  'no_rcr' => $no_rcr,'gender'=>$gender, 'position_name' => $position_name, 'position_publish'=>$position_publish,
  'target_applicant'=>$target_applicant,'target_fresh'=>$target_fresh,'target_exp'=>$target_exp, 'actual_fresh' => $actual_fresh,
  'actual_exp' => $actual_exp, 'target_pg_fresh' => $target_pg_fresh, 'target_pg_exp' => $target_pg_exp,
  'actual_pg_fresh' => $actual_pg_fresh, 'actual_pg_exp' => $actual_pg_exp, 'awaiting_fresh' => $awaiting_fresh,
  'awaiting_exp' => $awaiting_exp, 'target_pass_fresh' => $target_pass_fresh, 'target_pass_exp' => $target_pass_exp,
  'actual_pass_fresh' => $actual_pass_fresh, 'actual_pass_exp' => $actual_pass_exp,
